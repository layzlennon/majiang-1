cc.Class {
    ...
    init: () ->
        ...
        onTouchStart = (event) ->
            ...
            posY = this.node.getPositionY()
            posX = this.node.getPositionX()
            plusHeigth = self._viewSize.height * 0.2
            self.node.setPosition(cc.p(posX, posY + plusHeigth))
            self.node.emit "touchTile" , self._tileID
        onTouchMove = (event) ->
        ...
        this.node.on "touchTile", (data) ->
            console.log "touch Tile " + JSON.stringify data.detail
            if data.detail is self._tileID
                return
            else
